...
impl SourceComposable for ArcComposer<RustSpecification> {
    type Source = ScopeContext;
    type Output = Option<GenericComposerInfo<RustSpecification>>;

    fn compose(&self, source: &Self::Source) -> Self::Output {

        let arg_ty = self.ty.maybe_first_nested_type_ref()?.maybe_first_nested_type_ref()?;
        let wrap_alloc = |expr| Expression::new_smth(expr, DictionaryExpr::Arc);
        let wrap_from = |expr| DictionaryExpr::from_arc(DictionaryExpr::deref_ref(expr));
        ...
        let from_arg_value_conversion = <RustSpecification as Specification>::value_expr_from(arg_0_name.clone(), &arg_ty, value_arg_expr)
            .compose(source);
        let to_arg_conversion = <RustSpecification as Specification>::value_expr_to(arg_0_name.clone(), &arg_ty, Expression::Clone(root_arg_expr.into()))
            .compose(source);

        let bindings = Depunctuated::from_iter([
            self.kind.binding_presentable(&aspect, &attrs, &lifetimes, SmartPointerPresentableContext::Ctor(ctor_arg_composer, wrap_alloc(Expression::new_smth(self.kind.is_once_lock().then(|| Expression::Empty).unwrap_or_else(|| from_arg_conversion), self.kind.dictionary_type())))),
