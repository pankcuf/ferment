...
impl SourceComposable for RcComposer<RustSpecification> {
    ...
    fn compose(&self, source: &Self::Source) -> Self::Output {
        ...
        let wrap_alloc = |expr| Expression::new_smth(expr, DictionaryExpr::Rc);
        let wrap_from = |expr| DictionaryExpr::from_rc(DictionaryExpr::deref_ref(expr));
        ...
        let bindings = Depunctuated::from_iter([
            self.kind.binding_presentable(&aspect, &attrs, &lifetimes, SmartPointerPresentableContext::Ctor(ctor_arg_composer, wrap_alloc(Expression::new_smth(from_arg_conversion, self.kind.dictionary_type())))),
