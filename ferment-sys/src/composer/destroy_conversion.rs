            ...
            _ => {
                match composition {
                    ...
                    TypeModelKind::Optional(..) => {
                        let (expr_kind, target_ty) = match full_type.maybe_first_nested_type_kind()? {
                            TypeKind::Primitive(ty) =>
                                (ConversionExpressionKind::PrimitiveOpt, ty),
                            type_kind =>
                                (ConversionExpressionKind::ComplexOpt, type_kind.to_type())
                        };
                        Some(SPEC::Expr::cast_destroy(field_path, expr_kind, ffi_type, target_ty))
                    }
