...
impl<'a, SPEC> Composer<'a> for BoundsComposer<RustFermentate, SPEC>
    ...
    fn compose(&self, source: &'a Self::Source) -> Self::Output {
        ...
        self.model
            ...
            .for_each(|(index, predicate_ty)| {
                ...
                let (ty, args) = dictionary_generic_arg_pair::<RustFermentate, SPEC>(name.clone(), Name::Index(index), predicate_ty, &source);
