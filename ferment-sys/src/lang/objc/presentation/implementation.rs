...
impl Display for ImplementationPresentation {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_str(implementation.to_string().as_str())
...
impl Display for Implementation {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self { name, category, body } = self;
        let category = category.as_ref().map(|c| quote!((#c))).unwrap_or_default();
        f.write_str(quote!(@implementation #name #category).to_token_stream().to_string().as_str())?;
        f.write_str("\n")?;
