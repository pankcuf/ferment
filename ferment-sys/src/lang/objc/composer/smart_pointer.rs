    ...
    fn compose(&self, _source: &Self::Source) -> Self::Output {
        ...
        let bindings = {
            #[cfg(feature = "accessors")]
            {
                ...
                let ctor_arg_var = <ObjCSpecification as Specification>::value_var(arg_ty).compose(_source);
                let root_var = <ObjCSpecification as Specification>::value_var(root_ty_ref).compose(_source);
