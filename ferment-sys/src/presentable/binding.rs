...
use crate::composer::{BindingAccessorContext, CommaPunctuatedArgKinds, AspectArgComposers, NameKind, ArgKindPair, OwnerAspectSequence, SemiPunctuatedArgKinds, VariableComposer, SourceComposable, CommaPunctuatedArgs};
use crate::context::ScopeContext;
use crate::conversion::SmartPointerKind;
use crate::ext::{Mangle, ToPath, ToType};
use crate::lang::{RustSpecification, Specification};
use crate::presentable::{ArgKind, Aspect, ScopeContextPresentable, SeqKind};
...
pub enum BindingPresentableContext<SPEC>
    ...
    // Mutex(Aspect<SPEC::TYC>, SPEC::Attr, SPEC::Lt, SmartPointerPresentableContext<SPEC>),
    // RwLock(Aspect<SPEC::TYC>, SPEC::Attr, SPEC::Lt, SmartPointerPresentableContext<SPEC>)
}
...
impl<SPEC> BindingPresentableContext<SPEC>
    ...
    // pub fn mutex(aspect: &Aspect<SPEC::TYC>, attrs: &SPEC::Attr, lifetimes: &SPEC::Lt, lock_context: SmartPointerPresentableContext<SPEC>) -> Self {
    //     BindingPresentableContext::Mutex(aspect.clone(), attrs.clone(), lifetimes.clone(), lock_context)
    // }
    // pub fn rwlock(aspect: &Aspect<SPEC::TYC>, attrs: &SPEC::Attr, lifetimes: &SPEC::Lt, lock_context: SmartPointerPresentableContext<SPEC>) -> Self {
    //     BindingPresentableContext::RwLock(aspect.clone(), attrs.clone(), lifetimes.clone(), lock_context)
    // }
