...
impl WalletEx {
    ...
    pub fn select_coins_grouped_by_addresses(
        ...
        let vec_tally_ret = self.provider.select_coins(skip_denominated, anonymizable, skip_unconfirmed, max_outpoints_per_address);
    ...
    fn fresh_receive_key(&mut self, internal: bool) -> Vec<u8> {
        let fresh_key = self.provider.get_fresh_coinjoin_address(internal);
