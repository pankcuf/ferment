...
pub fn ffi_external_chunk<T: FFIResolver>(crate_ident: &Ident, segments: &Punctuated<PathSegment, Colon2>) -> T {
    ...
    // println!("ffi_external_chunk: {} -> {}", segments.to_token_stream(), quote!(crate::fermented::types::#crate_ident::#ffi_chunk_path));
...
// use syn::{Attribute, Item, parse_quote, Path, PathSegment, TraitBound, Type, TypeArray, TypeParamBound, TypePath, TypePtr, TypeReference, TypeSlice, TypeTraitObject, TypeTuple};
// use syn::punctuated::Punctuated;
// use syn::token::Colon2;
...
//                 let ffi_type = if let Some(ObjectConversion::Type(TypeCompositionConversion::Trait(tc, ..)) |
//                                            ObjectConversion::Type(TypeCompositionConversion::TraitType(tc))) = self.trait_ty(&ty) {
//                     let trait_ty = &tc.ty;
//                     let trait_path: Path = parse_quote!(#trait_ty);
//                     ffi_chunk_converted(&trait_path.segments)
//                 } else {
