...
impl From<&Type> for VariableComposer {
    fn from(value: &Type) -> Self {
        // println!("VariableComposer::from({})", value.to_token_stream());
        Self { ty: value.clone() }
    }
}
...
impl<'a> Composer<'a> for VariableComposer {
    ...
    fn compose(&self, source: &Self::Source) -> Self::Result {
        ...
        // println!("VariableComposer (compose): {} ({}) in {}", self.ty.to_token_stream(), full_ty.to_token_stream(), source.scope);
                        ...
                        FFIVariable::MutPtr {
                            ty: match &source.scope.parent_scope().unwrap().self_scope().object {
                                ...
                                ObjectConversion::Item(ref ty_conversion, ..) => {
                                    ...
                                    // println!("VariableComposer (Function Parent): {} ({}) in [{}]", ty_conversion.to_token_stream(), full_parent_ty.to_token_stream(), source.scope);
