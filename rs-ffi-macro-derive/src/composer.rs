...
use crate::{destroy_array, destroy_path, destroy_ptr, destroy_reference, from_array, from_path, from_ptr, from_reference, to_array, to_path, to_ptr, to_reference};
...
impl RootComposer {
    ...
    pub(crate) fn make_expansion(&self, input: TokenStream2) -> Expansion {
        // println!("RootComposer.expand: fields_from: {}", self.fields_from());
        // println!("RootComposer.expand: compose_from: {}", self.compose_from());
        // println!("RootComposer.expand: compose_to: {}", self.compose_to());
        // println!("RootComposer.expand: compose_destroy: {}", self.compose_destroy());
        // println!("RootComposer.expand: compose_drop: {}", self.compose_drop());
        Expansion::Full {
            input,
...
impl ConversionComposer {
    ...
    pub fn add_conversion(&mut self, name: TokenStream2, conversion: TokenStream2) {
        // println!("ConversionComposer::add_conversion({}, {})", &name, &conversion);
...
impl Composer for ConversionComposer {
    ...
    fn compose(&self) -> TokenStream2 {
        let context = (self.context_presenter)(&*self.parent.as_ref().unwrap().borrow());
        // println!("ConversionComposer::compose::context: {}", &context);
        // println!("ConversionComposer::compose::conversions ->: {}, {:?}", &self.path, &self.conversions);
        let conversions = (self.conversions_presenter)((self.path.to_token_stream(), self.conversions.clone()));
        // println!("ConversionComposer::compose::conversions <-: {}", &conversions);
        let composition = (self.composer)(context, conversions);
        // println!("ConversionComposer::compose::composition: {}", &composition);
        composition
    }
}
...
impl FieldsComposer {
    ...
    pub fn add_conversion(&mut self, name: TokenStream2, field_type: &Type) {
        // println!("ADD_CONVERSION (Fields) --->: {} {}", &name, quote!(#field_type));
        let value = (self.field_presenter)(name, field_type);
        // println!("ADD_CONVERSION (Fields) <---: {}", &value);
        self.fields.push(value);
    }

    // pub fn compose_mangled_type(&self, field_type: &Type) {
    //
    // }
}
impl Composer for FieldsComposer {
    ...
    fn compose(&self) -> TokenStream2 {
        let context = (self.context_presenter)(&*self.parent.as_ref().unwrap().borrow());
        // println!("COMPOSE (Fields) <---: {} {:?}", &context, self.fields.clone());
        let c = (self.root_presenter)((context, self.fields.clone()));
        // println!("COMPOSE (Fields) --->: {}", &c);
        c
    }
}
