...
use crate::{generics, path_arguments_to_path_conversions, path_arguments_to_paths, path_arguments_to_types};
...
/// Field Presenters
// pub const UNIT_VARIANT_FIELD_PRESENTER: FieldPresenter = |_| quote!();
        ...
        Type::Ptr(TypePtr { star_token, const_token, mutability, elem }) =>
            match &**elem {
                Type::Path(TypePath { path, .. }) => match path.segments.last().unwrap().ident.to_string().as_str() {
                    ...
                    _ => quote!(*mut #path)
                        // FFI_STRUCT_PATH_PRESENTER(path)
                        //panic!("extract_struct_field: ptr {:?} not supported {:?} {:?} {:?}", ptr, star_token, const_token, mutability)
                    // }
                },
                Type::Ptr(type_ptr) => quote!(*mut #type_ptr),
                // Type::Ptr(TypePtr { star_token, const_token, mutability, elem }) => match &**elem {
                //     Type::Path(TypePath { path, .. }) => FFI_STRUCT_PATH_PRESENTER(path),
                //     other_type => panic!("extract_struct_field: *mut *mut {:?} not supported", other_type)
                // },
...
// pub const FFI_VEC_FIELD_TYPE_MAP_PRESENTER: MapPresenter = |value_type| quote!(*mut rs_ffi_interfaces::VecFFI<#value_type>);
...
pub const EMPTY_PAIR_PRESENTER: MapPairPresenter = |_, _|
    quote!();
// pub const FFI_MAP_FIELD_TYPE_MAP_PAIR_PRESENTER: MapPairPresenter = |key_type, value_type| quote!(*mut rs_ffi_interfaces::MapFFI<#key_type, #value_type>);
...
pub const ENUM_UNIT_FIELDS_PRESENTER: OwnerIteratorPresenter = |(name, fields)|
    quote!(#name = #(#fields)*);
// pub const MAP_FIELDS_PRESENTER: OwnerIteratorPresenter = |(name, fields)| {
//     let count_conversion = fields.first().unwrap();
//     let key_conversion = fields.get(1).unwrap();
//     let value_conversion = fields.get(2).unwrap();
//     quote! {
//                                 (0..#count_conversion).fold(#name::new(), |mut acc, i| {
...
// pub const VEC_ARGUMENTS_PRESENTER: PathArgumentsPresenter = |arguments| match crate::path_arguments_to_paths(arguments)[..] {
//     [path] => FFI_VEC_FIELD_TYPE_MAP_PRESENTER(VEC_ARG_TYPE_PATH_PRESENTER(path)),
//     _ => panic!("VEC_ARGUMENTS_PRESENTER: arguments: {:?} not supported", arguments)
// };
// pub const MAP_ARGUMENTS_PRESENTER: PathArgumentsPresenter = |arguments| match crate::path_arguments_to_paths(arguments)[..] {
//     [path_keys, path_values] => FFI_MAP_FIELD_TYPE_MAP_PAIR_PRESENTER(MAP_ARG_TYPE_PATH_PRESENTER(path_keys), MAP_ARG_TYPE_PATH_PRESENTER(path_values)),
//     _ => panic!("MAP_ARGUMENTS_PRESENTER: arguments: {:?} not supported", arguments)
// };
...
// pub const GENERIC_MACRO_EXPANSION: MapPairPresenter = |method, arguments|
//     quote!(rs_ffi_interfaces::#method!(#arguments););
//
// pub const GENERIC_VEC_MACRO_EXPANSION: GenericVecPresenter = |(ffi_name, value_path)|
//     quote!(#ffi_name, #value_path);
//
// pub const GENERIC_MAP_MACRO_EXPANSION: GenericMapPresenter = |(ffi_name, root, key_path, value_path)|
//     quote!(#ffi_name, #root, #key_path, #value_path);
...
pub const GENERIC_VEC_SIMPLE_PRESENTER: GenericVecPresenter = |(ffi_name, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(vec_ffi_simple_expansion), GENERIC_VEC_MACRO_EXPANSION((ffi_name, value_path)));
...
pub const GENERIC_VEC_COMPLEX_PRESENTER: GenericVecPresenter = |(ffi_name, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(vec_ffi_complex_expansion), GENERIC_VEC_MACRO_EXPANSION((ffi_name, &mangle_path(value_path))));
...
pub const GENERIC_MAP_SIMPLE_PRESENTER: GenericMapPresenter = |(ffi_name, root, key_path, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(map_ffi_simple_expansion), GENERIC_MAP_MACRO_EXPANSION((ffi_name, root, key_path, value_path)));
...
pub const GENERIC_MAP_SIMPLE_COMPLEX_PRESENTER: GenericMapPresenter = |(ffi_name, root, key_path, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(map_ffi_simple_complex_expansion), GENERIC_MAP_MACRO_EXPANSION((ffi_name, root, key_path, &mangle_path(value_path))));
...
pub const GENERIC_MAP_COMPLEX_SIMPLE_PRESENTER: GenericMapPresenter = |(ffi_name, root, key_path, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(map_ffi_complex_simple_expansion), GENERIC_MAP_MACRO_EXPANSION((ffi_name, root, &mangle_path(key_path), value_path)));
...
pub const GENERIC_MAP_COMPLEX_PRESENTER: GenericMapPresenter = |(ffi_name, root, key_path, value_path)|
    ...
    // GENERIC_MACRO_EXPANSION(quote!(map_ffi_complex_expansion), GENERIC_MAP_MACRO_EXPANSION((ffi_name, root, &mangle_path(key_path), &mangle_path(value_path))));
...
// pub const VEC_PATH_PRESENTER: PathPresenter = |path| VEC_ARGUMENTS_PRESENTER(&path.segments.last().unwrap().arguments);
// pub const MAP_PATH_PRESENTER: PathPresenter = |path| MAP_ARGUMENTS_PRESENTER(&path.segments.last().unwrap().arguments);
